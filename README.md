...
The project includes a completely standalone web interface to visualize inlining decisions and LLVM IR changes.

You don't need any backend or local server to run the UI! Just open the HTML file directly in your browser:
...
Or just double-click `static/index.html` in your file explorer.

> **Note:** If you modify the pass and want to update the UI with your new results, run `python3 export_data.py` to regenerate the frontend data.
...
| Phase | What happens |
|-------|--------------|
| **0 — Sanitize** | Strip LLVM lifetime intrinsics that Clang `-O0` emits, which act as artificial inline blockers. |
| **1 — Analyze**  | Calculate instruction count, call count, cost, and detect recursive cycles via Call Graph DFS. |
| **2 — Inline**   | Walk the `ToInline` queue and attempt `InlineFunction()` on all caller sites. |
| **3 — Function DCE** | Erase dead functions that have zero callers (repeats until stable). |
| **4 — Instruction DCE** | Erase dead instructions that produce unused results and have no side effects (repeats until stable). |
...
Run `./run.sh`, showing all 11 tests passing with the baseline comparison table at the bottom.
...
Open `static/index.html`, select `chain_inline.ll`. Shows the decision cards (INLINE for all three), the before/after IR panels side by side with diff highlighting, and the metrics row showing 4→1 functions.
...
Switch to the Summary/All Tests tab. Shows the full cross-test table with all 11 files, their inlined/skipped/blocked columns, and IR reduction percentages.
...
tests/
  ...
  only_dce.c            Test 11: dead function and instruction removal
...
static/
  ...
  data.js               Pre-computed UI data generated by export_data.py
export_data.py          Script to bake LLVM IR outputs into static/data.js
